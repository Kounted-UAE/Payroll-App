...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return NextResponse.redirect(`${req.nextUrl.origin}/advontier/settings?teamwork_error=token_exchange_failed`)
    }
    ...
    return NextResponse.redirect(`${req.nextUrl.origin}/advontier/settings?teamwork_error=callback_failed`)
  }

  return NextResponse.redirect(`${req.nextUrl.origin}/advontier/settings?teamwork_connected=1`)
}
